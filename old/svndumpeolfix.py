...
class SvnDumpNode:
    ...
    def setActionReplaceFileObj( self, properties, fileObj, offset, length, md5,
                                fromPath="", fromRev=0 ):
    ...
    def convertEndOfLines( self, outfilename ):
        "Writes the text to the given file"
        ...
        while cnt > 0:
            ...
            if bcnt > 16384:
                bcnt = 16384
            str = self.fileObj.read( bcnt ).replace( "\r\n", "\n" ).replace( "\r", "\n" )
            md.update( str )
            outfile.write( str )
            cnt = cnt - bcnt
        self.textLen = outfile.tell()
        self.textMD5 = md.hexdigest()
        self.fileOffset = 0
        self.fileObj = None
            ...
            if action == "delete":
                ...
                if kind == "dir":
                    ...
                    if action == "add":
                        ...
                        node.setActionReplaceFileObj( properties, self.file,
                                offset, textlen, textmd5, copypath, copyrev )
                else:
...
def fixEolStyleAndAddRevisionFromDump( srcdump, dstdump, textfiles ):
    ...
    # check of state is done in addRevision
    # add revision and revprops
    dstdump.addRevision( srcdump.getRevProps() )
    ...
    while index < nodeCount:
        node = srcdump.getNode( index )
        istextfile = textfiles.has_key( node.getPath() )
        if not istextfile and node.getProperties() != None:
            if node.getProperties().has_key("svn:eol-style"):
                istextfile = True
                textfiles[ node.getPath() ] = 1
        if istextfile:
            #print "convert node %s in r%d" % ( node.getPath(), srcdump.getRevNr() )
            if node.convertEndOfLines( "tmpnode_%d" % index ):
                print "  converted %s" %  node.getPath()
        dstdump.addNode( node )
        index = index + 1
...
def diffDumpFiles( filename1, filename2 ):
    "diff 2 dump files"
    ...
    # open dump files
    dump1.open( filename1 )
    dump2.open( filename2 )
    ...
    while hasrev1 and hasrev2:
        # check rev numbers
        if dump1.getRevNr() != dump1.getRevNr():
            print "ERROR: different revision numbers: %d / %d" % ( dump1.getRevNr(), dump1.getRevNr() )
            hasrev1 = False
            hasrev2 = False
            break
        # more... ++++++
    ...
    #    if dump1.getRevNr() == 0:
    #        # create new dump with revision 0
    #        dstdmp.createWithRev0( dstfile, dump1.getUUID(),
    #                    dump1.getRevDateStr() )
    #        hasrev = dump1.readNextRev()
    #    else:
    #        # create new dump starting with the same revNr as the original dump
    #        dstdmp.createWithRevN( dstfile, dump1.getUUID(),
    #                    dump1.getRevNr() )
    #    # now copy all the revisions
    #    while hasrev:
    #        dstdmp.addRevisionFromDump( dump1 )
    #        hasrev = dump1.readNextRev()
    #else:
    #    print "no revisions in the source dump '%s' ???" % srcfile
...
if __name__ == '__main__':
    ...
    if appname == "svndumpmerge.py":
        ...
        returnCode = fixEolStyle( sys.argv[1], sys.argv[2] )
    elif appname == "svndumpdiff.py":
        returnCode = diffDumpFiles( sys.argv[1], sys.argv[2] )
    else:
        print "appname error"
    sys.exit( returnCode )
